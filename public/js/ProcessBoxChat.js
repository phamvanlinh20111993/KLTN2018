			...
			//   }
			  ...
			  //this function can remove a array element.
 				...
 				//mới thêm lúc 11:54pm, 5/5/2018 line 170-172
					 ...
					 for(iii; iii >= 0; iii--)
					 {
						  if(popups[iii] != undefined)
						  {
								...
								document.getElementById(popups[iii]+"_scrollmsg").partnerid = popups[iii]
								document.getElementById(popups[iii]+"_scrollmsg").countnumscroll = 0//default = 0
								//load message
								document.getElementById(popups[iii]+"_scrollmsg").onscroll = function(){
									if(this.scrollTop == 0){
									   this.countnumscroll++;
									   if(!this.stoprq)
									   {
									   	takecontentMessage(this.partnerid, this.countnumscroll, 
									   		function(content){
									   			if(content.listmessage)
													...
													if(myMessages.length > 0){
														var partnerid = 0;
														if(MYID == myMessages[0].idA)
															partnerid = myMessages[0].idB
														else 
															partnerid = myMessages[0].idA

														var temp = document.getElementById(partnerid+"_content").innerHTML
														var scrollBoxChat = document.getElementById(partnerid+"_scrollmsg").offsetHeight
														document.getElementById(partnerid+"_content").innerHTML = ""
									 					var myInfo = {}, 	pInfo = {}
									 					...
									 					for(var ind = myMessages.length-1; ind >= 0; ind--){
										  					if(MYID == myMessages[ind].idA){//toi la nguoi gui
																Message_send(myMessages[ind].idB, myMessages[ind].time, myInfo.photo, myMessages[ind], 1)
										  					}else{
																Message_receiver(myMessages[ind].idA, myMessages[ind].time, pInfo.photo, myMessages[ind], 1)
										  					}
									 					}   
									 					//khoi phuc
									 					document.getElementById(partnerid+"_content").innerHTML += temp
									 					document.getElementById(partnerid+"_scrollmsg").scrollTop = scrollBoxChat/2
													}
									   	})
									   }
				...
				var takecontentMessage = function(id, limit, cb){
					$.ajax({
						  type: "POST",
						  url: "/languageex/user/messages",
						  data:{anotherid: id, limit: limit},
				...
				function register_popup(event, id, name, photo)
				{
					 ...
					 takeSettingMessage(id, function(setting)
					 {
						 //  console.log(setting)
						  ...
						  takecontentMessage(id, 0, function(content)
						  {
